...
router.get("/api/artists/creation/:creation/:token", async (req, res) => {
	let result = [];

	if (req.params.creation.match(/^[0-9]+$/) == null) {
	...
	await pool.getConnection().then((conn) => {
		conn.query("SELECT COUNT(*) AS count FROM user WHERE token = ?", [
			...
			conn.query("SELECT * FROM `group` WHERE creation_date LIKE ?", [
				`%${req.params.creation}%`,
			])
				.then((rows) => {
